...
impl Interpreter {
    pub fn new(file_path: Option<PathBuf>) -> Self {
        ...
        interpreter.function_definitions.insert("print".into(), Function {
    ...
    pub async fn interpret_function_def(&mut self, name: &str, params: &[String], body: &Statement) -> Result<(), CrabbyError> {
        ...
        for statement in &program.statements {
            match statement {
                Statement::AsyncFunction { name, params, body, return_type } => {
                    let future = self.handle_async_function(name, params, body, return_type.clone())?;
                    futures.push(future);
                }
                _ => {
    ...
    pub fn handle_function_call(&mut self, function: &str, arguments: &[Expression]) -> Result<Value, CrabbyError> {
        ...
        let result = match lambda_opt {
            Some(Value::Lambda(lambda)) => {
                self.handle_lambda_call(lambda.clone(), arguments)
            }
            _ => {
    ...
    pub fn handle_lambda_call(&mut self, lambda: Function, arguments: &[Expression]) -> Result<Value, CrabbyError> {
        for (param, arg) in lambda.params.iter().zip(arguments) {
            let arg_value = self.interpret_expression(arg.clone())?;
            self.env.borrow_mut().insert(param.clone(), arg_value);
        }

        if let Some(value) = self.interpret_statement((*lambda.body).clone())? {
            Ok(value)
        } else {
            Ok(Value::Void)
        }
    ...
    pub fn interpret_let_statement(&mut self, name: &str, value: &Expression) -> Result<(), CrabbyError> {
        let interpreted_value = self.interpret_expression(value.clone())?;
        let is_public = name.starts_with("pub ");
    ...
    pub fn interpret_var_statement(&mut self, name: &str, value: &Expression) -> Result<(), CrabbyError> {
        let interpreted_value = self.interpret_expression(value.clone())?;
        let is_public = name.starts_with("pub ");
        let var_name = if is_public {
            name.trim_start_matches("pub ").to_string()
    ...
    pub fn handle_print(&mut self, args: &[Expression]) -> Result<Value, CrabbyError> {
        ...
        let value = self.interpret_expression(args[0].clone())?;
    ...
    pub fn interpret_match(&mut self, value: &Expression, arms: &[MatchArm]) -> Result<Option<Value>, CrabbyError> {
        let match_value = self.interpret_expression(value.clone())?;

        for arm in arms {
            if self.pattern_matches(&match_value, &arm.pattern)? {
                return Ok(Some(self.interpret_expression(arm.body.clone())?));
            }
        }
    ...
    pub fn pattern_matches(&mut self, value: &Value, pattern: &Expression) -> Result<bool, CrabbyError> {
        match pattern {
            Expression::Pattern(pattern_kind) => match &**pattern_kind {
                PatternKind::Literal(expr) => {
                    let interpreted = self.interpret_expression((**expr).clone())?;
    ...
    pub fn interpret_where(mut self, expr: &Expression, condition: &Expression, _body: &Statement) -> Result<Value, CrabbyError> {
        let cond_value = self.interpret_expression(condition.clone())?;
        if let Value::Boolean(true) = cond_value {
            self.interpret_expression(expr.clone())
        } else {
            Ok(Value::Void)
        }
    }

    pub fn load_and_import_module(mut self, _import_name: &str, import_path: &str) -> Result<Module, CrabbyError> {
        ...
        for statement in &ast.statements {
            module_interpreter.interpret_statement(&statement.clone())?;
        }
    ...
    pub fn interpret_statement(&mut self, stmt: Statement) -> Result<Option<Value>, CrabbyError> {
        match stmt {
            Statement::FunctionDef { name, params, body, return_type: _, docstring: _, visibility } => {
                // let is_public = name.starts_with("pub ");
                // let func_name = if is_public {
                //     name.trim_start_matches("pub ").to_string()
                // } else {
            ...
            Statement::FunctionFun { name, params, body, return_type: _, docstring: _, visibility } => {
                // let is_public = name.starts_with("pub ");
                // let func_name = if is_public {
                //     name.trim_start_matches("pub ").to_string()
                // } else {
            ...
            Statement::ForIn { variable, iterator, body } => {
                ...
                if let Value::Integer(n) = iter_value {
                    for i in 0..n {
                        self.env.borrow().insert(variable.clone(), Value::Integer(i));
                        self.interpret_statement(*body)?;
                    }
                    Ok(None)
                } else {
        ...
        match expr {
            ...
            Expression::Call { function, arguments } => {
                ...
                for (param, arg) in func.params.iter().zip(arguments) {
                    ...
                    new_interpret.env.borrow_mut().insert(param.clone(), arg_value);
                }

                let result = match new_interpret.interpret_statement(*func.body.clone()) {
            ...
            Expression::Index { array, index } => {
                let array_value = self.interpret_expression((*array).clone())?;
                let index_value = self.interpret_expression((*index).clone())?;
